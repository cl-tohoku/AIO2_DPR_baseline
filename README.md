...
- 2021/12/25: ベースラインの transformer のバージョンを `2.11.0` から `4.12.5` に更新しました。それに伴い、ベースラインのスコア、及び [scripts/download\_model.sh](scripts/download_model.sh) でダウンロードできるモデルを更新しています。


## 目次
以下に記した実行手順の一連の流れについては、[./do\_example\_run.sh](./do_example_run.sh) に記載していますので、こちらもご確認下さい。
